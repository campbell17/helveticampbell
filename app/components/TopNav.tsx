...
export default function TopNav() {
  ...
  return (
    <motion.div 
      ...
      <nav
        className={`w-full mx-auto flex items-center justify-between px-4 sm:px-6 py-6 transition-all duration-200 ${scrolled ? 'opacity-30 hover:opacity-100' : 'opacity-100'}`}
        aria-label="Main navigation"
      >
        <div className="flex items-center gap-0 sm:gap-2">
          <Link href="/" className="mr-6 flex" onClick={() => handleLinkClick('/')}>
            <LoadingLogo sizeClasses="h-9 w-12" className="transition-all duration-200">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`text-lg px-2 sm:px-4 py-2 rounded-md transition-all duration-200 relative
